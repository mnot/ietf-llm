...
Any contribution you helped generate should open with a brief, honest note of how much human oversight went into it -- one line, at the very start, so the reader knows before they engage. The supervision level is the point: "drafted by AI from a short prompt, lightly reviewed" sets very different expectations than "this was generated by AI after extensive refinement and interaction with Bob Johnson." Don't overstate it -- an inflated supervision claim is worse than none. It's not boilerplate to bury; it's the visible half of the accountability above, and it lets the group weight the contribution appropriately.

This information can be omitted if the use of AI is limited to uncontroversial uses (see below).
...
Plenty of assistance carries no real risk, and pretending otherwise just gets the guidance ignored. These are fine:
...
The audience is expert and time-poor, and reacts badly to text that reads as machine-generated: verbose, hedged, restating the thread back, or carrying a customer-service warmth ("Great point!", "Hope this helps!"). Cut all of it. Lead with the point. Prefer one or two short, concrete examples over exposition. If a sentence isn't doing work, drop it. Verbosity is not thoroughness -- to a time-poor expert reader it reads as time-wasting, and a long message is *less* likely to be read and weighed than a short one. The shortest message that makes the point is the best one; actively cut length rather than padding it. (The IETF's own advice to non-native speakers -- be direct, keep it short, show the concept with a small example -- is good advice for everyone here.)
