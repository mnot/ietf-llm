...
#: Filename parser for our local output. Mirrors the Datatracker
#: convention so a glob over `<wg>-polls-*.md` lines up neatly.
_LOCAL_POLLS_RE = re.compile(
    r"^(?P<wg>[a-z0-9_-]+?)-polls-(?P<meeting>\d+)-(?P<dt>\d{12})\.md$",
    re.IGNORECASE,
)
...
def fetch_polls_from_materials_page(
    ...
    for anchor in soup.find_all("a"):
        ...
        path = _local_polls_path(dest, wg, meeting_num, session_dt)
        if os.path.exists(path):
            log(
                f"Skipping polls {os.path.basename(path)}: already cached.",
                verbose, level=LogLevel.PROGRESS,
            )
            continue
...
def discover_local_polls(cache_dir: str) -> List["LocalPoll"]:
    """Walk the cache and return every `<wg>-polls-…md` file we've
    already cached. Used by the timeline digest to emit `poll` events
    ...
    for name in sorted(os.listdir(cache_dir)):
        match = _LOCAL_POLLS_RE.match(name)
        if not match:
            continue
...
def _local_polls_path(
    cache_dir: str, wg: str, meeting_num: str, session_dt: str
) -> str:
    return os.path.join(
        cache_dir, f"{wg}-polls-{meeting_num}-{session_dt}.md"
    )
