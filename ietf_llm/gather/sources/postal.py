...
def _is_geographic(stripped: str) -> bool:
    ...
    return (
        ...
        or _STREET_NUMBER_RE.match(stripped) is not None
...
def parse_document_year(text: str) -> Optional[int]:
    ...
    block, within the first couple of dozen lines: `16 May 2025` on a
    draft, `September 2024` on an RFC. The left-hand column often carries
    an `Expires` date six months later — and on older drafts both sit on
    the *same* line — so the header is split into columns and any column
    introduced by `Expires` is dropped before the scan.
    ...
    for raw in text.splitlines()[:30]:
        for column in _COLUMN_SPLIT_RE.split(raw.strip()):
            if not column or column.lower().startswith("expires"):
                continue
