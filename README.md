...
The fastest way to use `ietf-notebook` is to register its MCP server with
...
1. **Install with the search + MCP extras** so embeddings and the MCP
   ...
   ```bash
   pipx install 'ietf-notebook[search,mcp]'
   ```

   (If you already have it installed: `pipx install --force 'ietf-notebook[search,mcp]'`.)

2. **Register the MCP server** with your Claude client. For Claude Code:

   ```bash
   claude mcp add ietf-notebook -- ietf-notebook-mcp
   ```
   ...
   If `ietf-notebook-mcp` isn't on the PATH that Claude Desktop sees
   (common with `pipx`), use the absolute path instead, e.g.
   `"command": "/Users/you/.local/bin/ietf-notebook-mcp"` (find it with
   `which ietf-notebook-mcp`).
...
3. **(Recommended)** Install the bundled skill so Claude knows the
   ...
   ```bash
   git clone https://github.com/mnot/ietf-notebook /tmp/ietf-notebook
   cp -r /tmp/ietf-notebook/skill/ietf-notebook ~/.claude/skills/
   ```
...
Everything is written to `~/.cache/ietf-notebook/<wg>/` — the cache,
the digests, the embedding DB. The MCP server, `ietf-notebook-search`,
...
```bash
ietf-notebook --update httpbis --embed
```
...
```bash
ietf-notebook [OPTIONS] _wg_shortname_
```

This populates the local cache at `~/.cache/ietf-notebook/<wg>/` with the
WG charter, meeting minutes, slides, transcripts, mailing list archives,
and GitHub issues. The MCP server, `ietf-notebook-search`, and `--create`
...
```bash
ietf-notebook --destination _destination_ _wg_shortname_
```
...
(install with `pipx inject ietf-notebook llm`, or use the `summarize`
...
```bash
ietf-notebook-search httpbis "skepticism about cookie partitioning" -k 8
```
...
lives at `~/.cache/ietf-notebook/<wg>/embeddings.db` and updates
incrementally on the next `--embed` run. Requires the `search` extra:

```bash
pipx inject ietf-notebook 'ietf-notebook[search]'
```
...
`ietf-notebook-mcp` is a [Model Context Protocol](https://modelcontextprotocol.io/)
...
A Claude Code skill is included at `skill/ietf-notebook/SKILL.md`. Copy it
into your skills directory to let Claude gather and query WG corpora on your
behalf:

```bash
cp -r skill/ietf-notebook ~/.claude/skills/
```

The skill teaches Claude to drive the CLI, read the digest files first,
prefer `ietf-notebook-search` / the MCP tools over raw reads, and avoid
