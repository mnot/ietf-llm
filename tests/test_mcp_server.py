"""Tests for the pure tool-function bodies in the ietf_llm.mcp package.
...
def test_safe_path_resolves_valid_file(isolated_home: Path) -> None:
    ...
    resolved = mcp.common._safe_path("wg", "ok.txt")
...
def test_safe_path_rejects_traversal(isolated_home: Path) -> None:
    ...
    assert mcp.common._safe_path("wg", "../../../etc/passwd") is None
    assert mcp.common._safe_path("wg", "../../other-wg/files/x.txt") is None


def test_safe_path_returns_none_for_missing(isolated_home: Path) -> None:
    write_cache_file(isolated_home, "wg", "exists.txt")
    assert mcp.common._safe_path("wg", "missing.txt") is None
...
def test_read_file_section_respects_max_lines(isolated_home: Path) -> None:
    ...
    out = mcp.chunks.tool_read_file_section("wg", "long.txt", start_line=1, max_lines=10)
...
def test_read_file_section_rejects_oversized_request(isolated_home: Path) -> None:
    ...
    out = mcp.chunks.tool_read_file_section("wg", "ok.txt", max_lines=99999)
...
def test_read_file_section_supports_start_line(isolated_home: Path) -> None:
    ...
    write_cache_file(isolated_home, "wg", "long.txt", content)
    out = mcp.chunks.tool_read_file_section(
...
def test_read_file_section_rejects_path_traversal(isolated_home: Path) -> None:
    write_cache_file(isolated_home, "wg", "ok.txt", "hi")
    out = mcp.chunks.tool_read_file_section("wg", "../../../etc/passwd")
...
def test_list_corpora_states_session_facts(
    ...
    on = mcp.corpus.tool_list_corpora()
    ...
    off = mcp.corpus.tool_list_corpora()
...
def test_read_digest_people_kind_is_valid(isolated_home: Path) -> None:
    ...
    out = mcp.digest.tool_read_digest("wg", "people")
...
def test_read_digest_rejects_unknown_kind(isolated_home: Path) -> None:
    ...
    out = mcp.digest.tool_read_digest("wg", "nonsense")
...
def test_read_digest_absent_kind_reports_what_corpus_has(isolated_home: Path) -> None:
    ...
    write_cache_file(isolated_home, "wg", "digests/people.md", "# people\n")
    out = mcp.digest.tool_read_digest("wg", "issues")
...
def test_read_digest_sort_activity_wired(isolated_home: Path) -> None:
    ...
    out = mcp.digest.tool_read_digest("wg", "threads", sort="activity")
...
def test_read_digest_exclude_mechanical_wired(isolated_home: Path) -> None:
    ...
    out = mcp.digest.tool_read_digest("wg", "timeline", exclude_mechanical=True)
...
def test_read_digest_no_digests_at_all(isolated_home: Path) -> None:
    ...
    out = mcp.digest.tool_read_digest("wg", "threads")
...
def _nonexistent_corpus_calls(nx: str):
    ...
    return {
        "overview": lambda: mcp.corpus.tool_overview(nx),
        "read_digest": lambda: mcp.digest.tool_read_digest(nx, "threads"),
        "search": lambda: mcp.search.tool_search(nx, "q"),
        "list_labels": lambda: mcp.corpus.tool_list_labels(nx),
        "list_files": lambda: mcp.corpus.tool_list_files(nx),
        "read_topic": lambda: mcp.topic.tool_read_topic(nx, "q"),
        "tally_positions": lambda: mcp.topic.tool_tally_positions(nx, "threads/x.md"),
        "find_replies": lambda: mcp.topic.tool_find_replies(nx, "threads/x.md", 1),
        "find_citations": lambda: mcp.citations.tool_find_citations(nx, "draft-x"),
        "find_message_citations": lambda: mcp.citations.tool_find_message_citations(
            nx, "threads/x.md"
        ),
        "get_chunk": lambda: mcp.chunks.tool_get_chunk(nx, "threads/x.md", 1),
        "get_chunks_batch": lambda: mcp.chunks.tool_get_chunks_batch(
            nx, [{"file": "threads/x.md", "chunk_idx": 1}]
        ),
        "get_by_url": lambda: mcp.chunks.tool_get_by_url(
            nx, "https://www.w3.org/mid/x"
        ),
        "read_file_section": lambda: mcp.chunks.tool_read_file_section(nx, "charter.txt"),
    }
...
def test_invalid_date_rejected_not_silently_empty(isolated_home: Path) -> None:
    ...
    for bad in ("not-a-date", "2026-13-40", "05/01/2026"):
        assert "Invalid" in mcp.digest.tool_read_digest("wg", "threads", since=bad)
        assert "Invalid" in mcp.digest.tool_read_digest("wg", "threads", until=bad)
        assert "Invalid" in mcp.search.tool_search("wg", "q", since=bad)
    # A valid date is not rejected.
    assert "Invalid" not in mcp.digest.tool_read_digest(
...
def test_search_k_clamped_no_crash(isolated_home: Path) -> None:
    ...
    for bad_k in (100000, -5, 0):
        out = mcp.search.tool_search("wg", "anything", k=bad_k)
...
def test_collapse_draft_versions() -> None:
    from ietf_llm.mcp.search import _collapse_draft_versions
...
def test_offload_times_out_slow_calls(monkeypatch: object) -> None:
    ...
    async def run(fn: object) -> str:
        return await mcp.common._offload(fn)
...
def test_offload_timeout_disabled_by_zero(monkeypatch: object) -> None:
    ...
    async def run() -> str:
        return await mcp.common._offload(quick)
...
def test_offload_emits_access_record_at_status(
    ...
    async def run() -> str:
        return await mcp.common._offload(tool_overview)
...
def test_top_level_response_flags_an_in_flight_refresh(
    ...
    clean = mcp.digest.tool_read_digest("wg", "people")
...
def test_inflight_note_omits_zero_index_stage(
    ...
    note = mcp.common._inflight_refresh_note("wg")
    ...
    bare = mcp.common._inflight_refresh_note("wg")
    assert bare is not None and "in progress" in bare


def test_stage_phrase_renders_or_none() -> None:

    assert mcp.common._stage_phrase(None) is None
    assert mcp.common._stage_phrase({"stage_index": 0, "stage_total": 19}) is None
    assert (
        mcp.common._stage_phrase(
            {"stage_index": 18, "stage_total": 19, "stage": "embedding index"}
        )
        == "stage 18/19 (embedding index)"
    )
    assert mcp.common._stage_phrase({"stage": "mailing list"}) == "stage: mailing list"
...
def test_first_gather_guard_refuses_when_never_gathered(
    ...
    msg = mcp.common._first_gather_guard("wg")
...
def test_timeout_note_names_running_gather(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    note = mcp.common._timeout_inflight_note(mcp.search.tool_search, ("wg", "query"))
    assert note is not None
    ...
    assert mcp.common._timeout_inflight_note(mcp.search.tool_search, ("a query!", "q")) is None
    # Empty args (a no-corpus tool) -> no note.
    assert mcp.common._timeout_inflight_note(mcp.search.tool_search, ()) is None
    # No gather live -> no note.
    monkeypatch.setattr(gather_runner, "local_inflight", lambda wg: None)
    assert mcp.common._timeout_inflight_note(mcp.search.tool_search, ("wg", "query")) is None
...
def test_index_rebuilding_note(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(mcp.common, "probe_index", lambda wg: False)
    note = mcp.common._index_rebuilding_note("wg")
    ...
    monkeypatch.setattr(mcp.common, "probe_index", lambda wg: True)
    assert mcp.common._index_rebuilding_note("wg") is None
    # No gather live -> no note.
    monkeypatch.setattr(gather_runner, "local_inflight", lambda wg: None)
    assert mcp.common._index_rebuilding_note("wg") is None
...
def test_get_chunk_on_digest_file_redirects_to_read_digest(
    ...
    # right tool to call.
    write_cache_file(isolated_home, "wg", "digests/people.md", "# people\n")
    out = mcp.chunks.tool_get_chunk("wg", "digests/people.md", 0)
...
def test_get_chunk_unknown_file_explains_what_to_do(
    ...
    out = mcp.chunks.tool_get_chunk("wg", "not-indexed.md", 0)
...
def test_read_file_section_on_digest_includes_hint(
    ...
    out = mcp.chunks.tool_read_file_section("wg", "digests/issues.md")
    assert "read_digest" in out
...
def test_list_files_annotates_digest_files(isolated_home: Path) -> None:
    ...
    out = mcp.corpus.tool_list_files("wg")
...
def test_list_files_pattern_filter(isolated_home: Path) -> None:
    ...
    out = mcp.corpus.tool_list_files("wg", pattern="threads/*mlkem*")
...
def test_get_chunk_range_rejects_inverted_bounds(isolated_home: Path) -> None:
    ...
    out = mcp.chunks.tool_get_chunk("wg", "x.txt", 5, end_chunk_idx=2)
    assert "less than" in out


def test_get_chunk_range_caps_size(isolated_home: Path) -> None:
    write_cache_file(isolated_home, "wg", "x.txt", "hi")
    out = mcp.chunks.tool_get_chunk("wg", "x.txt", 0, end_chunk_idx=999)
...
def test_read_digest_include_bodies_appends_issue_files(
    ...
    out = mcp.digest.tool_read_digest(
...
def test_get_by_url_returns_chunk_when_url_matches(
    ...
    _build_with_stub("wg", isolated_home)
    out = mcp.chunks.tool_get_by_url(
...
def test_get_by_url_resolves_w3_mid_archived_at(isolated_home: Path) -> None:
    ...
    _build_with_stub("wg", isolated_home)
    out = mcp.chunks.tool_get_by_url("wg", mid)
...
def test_get_by_url_normalises_body_footnote_spelling(
    ...
    for variant in (
        ...
        out = mcp.chunks.tool_get_by_url("wg", variant)
...
def test_get_by_url_returns_helpful_miss_message(
    ...
    write_cache_file(isolated_home, "wg", "x.txt", "hi")
    out = mcp.chunks.tool_get_by_url("wg", "https://mailarchive.ietf.org/arch/msg/x/y/")
...
def test_get_chunks_batch_concatenates_multiple_files(
    ...
    _build_with_stub("wg", isolated_home)
    out = mcp.chunks.tool_get_chunks_batch("wg", [
...
def test_get_chunks_batch_rejects_non_numeric_index(
    ...
    out2 = mcp.chunks.tool_get_chunks_batch("wg", ["not-a-dict"])
    assert "must be an object" in out2


def test_descendants_is_cycle_safe() -> None:
    from ietf_llm.mcp.topic import _descendants# pylint: disable=import-outside-toplevel
...
def test_read_file_window_past_eof(tmp_path) -> None:
    ...
    path = tmp_path / "f.txt"
    path.write_text("a\nb\nc\n", encoding="utf-8")
    out = mcp.drafts._read_file_window(str(path), start_line=20, max_lines=100)
...
def test_load_server_instructions_is_markdown_heading() -> None:
    ...
    out = mcp.server._load_server_instructions()  # pylint: disable=protected-access
...
def test_load_server_instructions_prepends_session_when_marker_missing(
    ...
    monkeypatch.setattr(
        mcp.server, "_strip_frontmatter", lambda _t: "# Routing\n\nbody, no marker"
    )
    out = mcp.server._load_server_instructions()  # pylint: disable=protected-access
...
def test_read_ietf_interpretation_norms_returns_bundled_doc() -> None:
    ...
    out = mcp.norms.tool_read_interpretation_norms()
...
def test_read_ietf_participation_norms_returns_bundled_doc() -> None:
    ...
    out = mcp.norms.tool_read_participation_norms()
