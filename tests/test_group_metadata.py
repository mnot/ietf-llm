"""Tests for Datatracker group-metadata helpers in `groups`.
...
from ietf_llm import groups, utils
...
def _stub_api(
    ...
    """Stub groups.fetch_resource: group record by acronym, extresources
    by group id. Clears the per-process caches first."""
    groups.fetch_group_object.cache_clear()
    groups.get_group_resources.cache_clear()
    ...
    monkeypatch.setattr(groups, "fetch_resource", fake_fetch)
...
def test_mailing_list_name_primary_for_ietf_list(
    ...
    assert groups.get_mailing_list_name("tls") == "tls"
...
def test_mailing_list_name_falls_back_to_alternate_archive(
    ...
    assert groups.get_mailing_list_name("httpbis") == "httpbisa"
...
def test_mailing_list_name_external_without_alternate_keeps_primary(
    ...
    assert groups.get_mailing_list_name("foo") == "some-list"
...
def test_mailing_list_name_no_record_uses_shortname(
    ...
    assert groups.get_mailing_list_name("ghostwg") == "ghostwg"
...
def test_group_resources_parses_slug_label_and_value(
    ...
    by_slug = {slug: (label, value) for slug, label, value in
               groups.get_group_resources("httpbis")}
...
def test_group_state_and_area(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(groups, "fetch_resource", fake_fetch)
    assert groups.get_group_state("httpbis") == "active"
    assert groups.get_group_area("httpbis") == ("wit", "Web and Internet Transport")
...
def test_group_name(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(groups, "fetch_resource", fake_fetch)
    assert groups.get_group_name("httpbis") == "HTTP"
    groups.fetch_group_object.cache_clear()
    monkeypatch.setattr(groups, "fetch_resource", lambda *a, **k: None)
    assert groups.get_group_name("nope") is None
